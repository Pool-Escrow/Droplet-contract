...
contract Droplet is Ownable, IERC20, IERC20Metadata, IERC20Errors, IERC20Permit, EIP712, Nonces {
    ...
    function totalSupply(string memory slot_) public view virtual returns (uint256) {
        string memory key = "totalSupply";
        bytes32 storageSlot_ = bytes32(uint256(keccak256(abi.encodePacked(slot_))) - 1);
        return storageSlot_.deriveMapping(key).getUint256Slot().value;
    }
    ...
    function balanceOf(address account, string memory slot_) public view virtual returns (uint256) {
        bytes32 storageSlot_ = bytes32(uint256(keccak256(abi.encodePacked(slot_))) - 1);
        return storageSlot_.deriveMapping(account).getUint256Slot().value;
    }
    ...
    function _update(address from, address to, uint256 value) internal virtual {
        if (from == address(0)) {
            ...
            uint256 fromBalance = _getBalanceSlot(from).value;
            if (fromBalance < value) {
                revert ERC20InsufficientBalance(from, fromBalance, value);
            }
            unchecked {
                // Overflow not possible: value <= fromBalance <= totalSupply.
                _getBalanceSlot(from).value = fromBalance - value;
            }
        }
    ...
    function _getBalanceSlot(address account) internal view virtual returns (StorageSlot.Uint256Slot storage) {
    ...
    function _getTotalSupplySlot() internal view virtual returns (StorageSlot.Uint256Slot storage) {
        string memory key = "totalSupply";
        return storageSlot.deriveMapping(key).getUint256Slot();
    }
}
